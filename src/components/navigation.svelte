...
<header class="flex justify-between items-center p-4 bg-gray-800 text-white fixed w-dvw top-0 left-0">
	...
	<button class="transition ease-linear delay-100 fixed z-98 top-0 left-0 h-dvh w-dvw backdrop-brightness-{opened ? "50" : "0 hidden"}" onclick={toggle(false)} aria-label="layer"></button>
	<nav
		class={opened
